...
# render_template_string : 문자열로 작성한 jinja템플릿을 렌더링 해 html로 반환
# redirect : 다른 url로 이동하기 
# url_for : 라우트 함수 이름으로 url을 안전하게 생성하는 함수

app = Flask(__name__)

# cookie는 브라우저에 저장되는 작은 키-값 데이터, 
# 서버가 클라이언트와 연결 상태 유지하는 것처럼 할 수 있음
...
def set_cookie():
    ...
    # 먼저 "read_cookie" 페이지로 이동하라는 redirect객체를 만들고 
