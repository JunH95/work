...
# flash : 임시 메세지 출력용 (내부적으로 session에 저장해 둠 - securet_key 필요 )

app = Flask(__name__)
app.secret_key = "abcdef123456"   # session/flash를 위한 쿠키 서명용 비밀 키
...
def add_save():   # 추가 처리
    ...
    su_raw = (request.form.get("su") or "").strip()    # "23" 수가 문자열로 넘어옴
