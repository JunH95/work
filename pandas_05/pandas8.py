...
print(root)  #<Element '{current}current' at 0x000001D338B43A10>

# {current} namespace 제거
for elem in root.iter():
    if "}" in elem.tag:
        elem.tag = elem.tag.split("}", 1)[1]   # "}" 기준으로 잘라 태그명만 남김

# {current}weather -> weather
weather = root.find("weather")
year = weather.get("year")   # weather의 속성값 얻기
...
for local in weather.findall("local"):
    name = local.text.strip()   # 태크 안에 텍스트
    ta = local.get("ta")    # local 요소(엘리먼트)의 ta 속성(attribute)
