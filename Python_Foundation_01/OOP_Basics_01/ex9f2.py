...
def doFunc2(name):
    print("name : ", name)
    # return None 반환값이 없을땐 아무것도 안쓰거나 return None
...
def swapFunc(a, b):
    return b, a     # return (b, a) 리턴 값은 언제나 하나 튜플은 () 생략 가능 
...
player = "전국대표"        # 전역변수 (현재 모듈(파일) 어디서든 호출 가능)
name = "한국인"
...
def Foo():
    ...
    b = 100     
    def Bar():
        global c # Bar 함수의 멤버가 아니라 모듈(파일)의 멤버, 전역변수
        nonlocal b # inclosing func(Foo)의 멤버
        b = 8    # 지역변수
        print(f"함수 Bar 후 a:{a}, b:{b}, c:{c}")
        c = 9 
        b = 200  
    Bar()
    print(f"함수 Foo 후 a:{a}, b:{b}, c:{c}")
