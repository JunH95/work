...
# skip_blank_lines : 컬럼명, 데이터의 앞에 공백을 제거
df = pd.read_table("ex1.csv", sep=",", skip_blank_lines=True)
print(df)

pd.set_option("display.max_columns", None)  # 모든 컬럼 표시 옵션
...
for bar, time_var in zip(bars, times):
    plt.text(bar.get_x() + bar.get_width() / 2, bar.get_height(), f"{time_var:3f}초", ha="center", va="bottom", fontsize=10)
